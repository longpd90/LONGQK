...
- (BOOL)syncGET:(NSString *)URLString
    ...
    QKCSBigLoadingView *indicator;
    if (isLoading) {
        indicator = [[QKCSBigLoadingView alloc] initHUDView];
...
- (void)alertView:(CCAlertView *)alertView selectedButtonIndex:(NSInteger)index {
    if (index == 0) {
        ...
        [QKAccessUserDefaults put:@"QKAccessTokenInvalidKey" withValue:@"1"];
        UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"Main" bundle:[NSBundle mainBundle]];
        UIWindow *window = [[UIApplication sharedApplication] keyWindow];
        UIViewController *mainMenuNavigationViewController = [storyboard instantiateViewControllerWithIdentifier:@"QKNavigationSignupViewController"];
