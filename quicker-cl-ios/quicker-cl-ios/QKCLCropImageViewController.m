...
@implementation QKCLCropImageViewController
...
- (IBAction)cropImage:(id)sender {
    ...
    if (_mode == QKUploadModeProfile) {
        if ([self connected]) {
            ...
            [params setObject:@"0" forKey:@"deleteImageFlg"];
            
            [[QKCLRequestManager sharedManager] asyncPOST:[NSString stringFromConst:qkUrlProfileUpdate] parameters:params showLoading:YES showError:YES constructingBodyWithBlock: ^(id <AFMultipartFormData> formData) {
                if (self.croppedImage) {
			                 [formData appendPartWithFileData:UIImageJPEGRepresentation(self.croppedImage, 1.0f) name:@"imageFile" fileName:@"image.jpeg" mimeType:@"image/jpeg"];
                }
                ...
                if ([responseObject[QK_API_STATUS_CODE] isEqualToString:[NSString stringFromConst:QK_STT_CODE_SUCCESS]]) {
			                 [self dismissViewControllerAnimated:YES completion: ^{
                                 if (self.delegate && [self.delegate respondsToSelector:@selector(croppedImage:imageId:)]) {
			                                  [self.delegate croppedImage:self.croppedImage imageId:responseObject[@"imageId"]];
                                 }
                             }];
                }
        ...
        if ([self connected]) {
            ...
            [params setObject:_sourceImageType forKey:@"imageType"];
            
            [[QKCLRequestManager sharedManager] asyncPOST:[NSString stringFromConst:qkUrlShopImageUpload] parameters:params showLoading:YES showError:YES constructingBodyWithBlock: ^(id <AFMultipartFormData> formData) {
                if (self.croppedImage) {
			                 [formData appendPartWithFileData:UIImageJPEGRepresentation(self.croppedImage, 1.0f) name:@"imageFile" fileName:@"image.jpeg" mimeType:@"image/jpeg"];
                }
            } success: ^(AFHTTPRequestOperation *operation, id responseObject) {
