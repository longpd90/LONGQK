...
#define kQKCLAutoCloseAlertValue 2.5
...
- (id)initWithImage:(UIImage *)image
    ...
    if (self) {
        ...
        if (!_timer) {
            _timer = [NSTimer scheduledTimerWithTimeInterval:kQKCLAutoCloseAlertValue target:self selector:@selector(timerFired:) userInfo:nil repeats:NO
                      ];
        }
    }
...
- (void)createMessageDialog:(QKAlertViewStyle)styleAlertView {
    ...
    //init dialog
    _messageDialog = [[UIView alloc] initWithFrame:CGRectMake((_keyWindow.frame.size.width - width) / 2, 0.0, width, 0)];
    ...
    CGRect messageLabel = _messageLabel.frame;
    messageLabel.size.width = width;
    _messageLabel.frame = messageLabel;
    [_messageDialog addSubview:_messageLabel];
    
    if (!_messageLabel || [_message isEqualToString:@""]) {
        y = CGRectGetMaxY(_messageLabel.frame) + 20.0;
    }
...
- (void)createAlertViewHaveTextField:(BOOL)haveTextField {
    ...
    if (self.image) {
        ...
        [_messageDialog addSubview:_iconImageView];
        _yBottom = CGRectGetMaxY(_iconImageView.frame) + 14.0;
    }
    else if (_message) {
        _yBottom = 20.0;
    }
    ...
    _messageLabel.numberOfLines = 0;
    [_messageLabel setFont:[UIFont systemFontOfSize:11.0]];
    [_messageLabel sizeToFit];
    [_messageLabel setTextAlignment:NSTextAlignmentCenter];
    labelFrame = _messageLabel.frame;
    labelFrame.size.width = width;
    _messageLabel.frame = labelFrame;
    [_messageDialog addSubview:_messageLabel];
    if (_message == nil || [_message isEqualToString:@""]) {
        _yBottom  = CGRectGetMaxY(_messageLabel.frame);
        if (_image == nil) {
    ...
    if (haveTextField) {
        _yBottom -= 10.0;
        self.textField = [[UITextField alloc] initWithFrame:CGRectMake(10, _yBottom, width, 33.0)];
