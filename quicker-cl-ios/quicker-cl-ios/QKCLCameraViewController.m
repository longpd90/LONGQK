...
@interface QKCLCameraViewController ()
{
    UIDevice *device;
}
...
@implementation QKCLCameraViewController
...
- (void)viewDidLoad {
    ...
    NSError *error = nil;
    self.videoInput = [AVCaptureDeviceInput deviceInputWithDevice:dv error:&error];
    if (self.videoInput) {
...
- (void)rotateButtonsInCamera {
    if (device.orientation == UIDeviceOrientationPortrait) {
        ...
        [self.btnSwichCamera setImage:[UIImage imageNamed:@"camera_btn_self"] forState:UIControlStateNormal];
        ...
        //rotate image switch camera
        returnImage = [self imageRotatedByDegrees:[UIImage imageNamed:@"camera_btn_self"] deg:90];
        ...
        //rotate image switch camera
        returnImage = [self imageRotatedByDegrees:[UIImage imageNamed:@"camera_btn_self"] deg:180];
        [self.btnSwichCamera setImage:returnImage forState:UIControlStateNormal];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    //check camerahint
    if (_hintSegue == nil || [_hintSegue isEqualToString:@""]) {
        [_cameraHintButton setHidden:YES];
    }
    else {
        [_cameraHintButton setHidden:NO];
    }
}
...
- (void)swapFrontAndBackCameras {
    ...
    for (AVCaptureDeviceInput *input in inputs) {
        AVCaptureDevice *dv = input.device;
        if ([dv hasMediaType:AVMediaTypeVideo]) {
            AVCaptureDevicePosition position = dv.position;
...
- (AVCaptureDevice *)cameraWithPosition:(AVCaptureDevicePosition)position {
    ...
    for (AVCaptureDevice *dv in devices) {
        if (dv.position == position) {
            captureDevice = dv;
            break;
        }
    }
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqualToString:@"QKShowCropImageSegue"]) {
        QKCLCropImageViewController *vc = (QKCLCropImageViewController *)segue.destinationViewController;
    ...
    if ([segue.identifier isEqualToString:@"QKAccountFaceHintSegue"]) {
        QKCLAccountCameraHintViewController *vc = (QKCLAccountCameraHintViewController *)segue.destinationViewController;
        vc.isHint = YES;
    }
    if ([segue.identifier isEqualToString:@"QKShopHintSegue"]) {
        QKCLShopCameraHintViewController *vc = (QKCLShopCameraHintViewController *)segue.destinationViewController;
        vc.isHint = YES;
    }
}
